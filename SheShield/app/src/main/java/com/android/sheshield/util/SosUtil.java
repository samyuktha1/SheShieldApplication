package com.android.sheshield.util;
...
import com.android.sheshield.api.NotificationAPI;
import com.android.sheshield.common.Constants;
import com.android.sheshield.config.Prefs;
import com.android.sheshield.model.ContactModel;
import com.android.sheshield.model.NotificationDataModel;
...
//                                    Toast.makeText(context,message,Toast.LENGTH_LONG).show();
                                    ...
                                    if (Prefs.getBoolean(Constants.SETTINGS_SEND_SMS, true) && !sentSMS) {
                                        ...
                                        sendNotificationDialog(context);
                                    }
    ...
    private static void sendNotificationDialog(Context context) {
        AlertDialog.Builder dialogBuilder = new AlertDialog.Builder(context);

        // Set your dialog content here (e.g., message, buttons, etc.)
        AlertDialog dialog = dialogBuilder
                .setTitle("Your location is being shared")
                .setMessage("Please open your text message to confirm")
                ...
                .setNegativeButton("Cancel", new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        // You can add functionality for the Cancel button here
                        dialog.dismiss();
                    }
                })
                .create();

        // Set the dialog animation to slide up from the bottom
        dialog.getWindow().getAttributes().windowAnimations = R.style.SlideDialogAnimation;

        // Show the dim background overlay
        ImageView dimBackground = ((Activity) context).findViewById(R.id.imageViewDimBackground);
        dimBackground.setVisibility(View.VISIBLE);

        // Show the dialog
        dialog.show();

        // Dismiss the dim background overlay when the dialog is dismissed
        dialog.setOnDismissListener(dialogInterface -> dimBackground.setVisibility(View.INVISIBLE));
    }
